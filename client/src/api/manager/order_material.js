...
export const api_getOrderMaterialByManagerIdStatusWaiting = async () => {
    try {
        const {data:{user_id : manager_id}} = await axiosGet(`${ROOT_SERVER}/api/checkToken`);
        const {data} = await axiosGet(`${ROOT_SERVER}/api/manager/getOrderMaterialByManagerIdStatusWaiting?manager_id=${manager_id}`);
        return data;
    } catch (error) {
        console.error(error)
        await Swal.fire({
...
export const api_getOrderMaterialByManagerIdStatusConsidered = async () => {
    try {
        const {data:{user_id : manager_id}} = await axiosGet(`${ROOT_SERVER}/api/checkToken`);
        const {data} = await axiosGet(`${ROOT_SERVER}/api/manager/getOrderMaterialByManagerIdStatusConsidered?manager_id=${manager_id}`);
        return data;
    } catch (error) {
        console.error(error)
        await Swal.fire({
...
export const api_updateOrderMaterialToConsidered = async (formData) => {
    try {
        return InsertFormSwal(()=>axiosPost(`${ROOT_SERVER}/api/manager/updateOrderMaterialToConsidered`, {...formData}))
    } catch (error) {
        console.error(error)
        await Swal.fire({
            title: "ผิดพลาด",
